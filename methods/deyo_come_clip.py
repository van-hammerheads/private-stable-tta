...
class DeYO_COME_Clip(nn.Module):
    ...
    def __init__(self, model, args, optimizer, steps=1, episodic=False, deyo_margin=0.5*math.log(1000), margin_e0=0.4*math.log(1000)):
        ...
        self.noise = 0.0  # clip-only variant: DP noise is pinned to 0
...
def forward_and_adapt_deyo(x, iter_, model, args, optimizer, deyo_margin, margin, targets=None, flag=True, group=None):
    ...
    if backward == 0:
        # TMP: dummy loss and backward to make opacus keep calculating grad samples
        dummy_loss = (outputs * 0).sum()
        dummy_loss.backward()
        optimizer.step()
        optimizer.zero_grad()
